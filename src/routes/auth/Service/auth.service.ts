...
export class AuthService {
  ...
  async validateAuthorizeRequest(data: Authorize): Promise<void> {
    ...
    const redirectUriIsAllowed = project.redirectUris.some(
      (r) => r.redirectUri === data.redirect_uri,
    );
  ...
  async exchangeCodeForToken(token: Token): Promise<{
    ...
    const { data } = await this.redis.getAuthCode(token.code);

    if (!data) {
      throw new UnauthorizedException('Invalid or expired authorization code');
    }

    if (data.clientId !== token.client_id) {
      throw new UnauthorizedException('client_id mismatch');
    }

    const projectId = await this.getProjecIdByClientId(token.client_id);

    const computedChallenge = crypto
      .createHash('sha256')
      .update(token.code_verifier)
      .digest('base64url');
    ...
    if (token.redirect_uri !== data.redirectUri) {
